...
=head2 ( $tree, $xml, $code ) = $tpp->parsehttp( $method, $url, $body, $head );
...
C<$tree> is a referecen to a hash tree.
...
C<UTF-8>. The default value is C<UTF-8>.
...
=head2 $tpp->set( base_class => 'MyElement' );
...
=head2 $tpp->set( elem_class => 'MyElement' );
