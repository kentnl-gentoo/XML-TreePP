...
sub write {
    ...
    my $to = $self->{output_encoding} || $XML_ENCODING;
    my $decl = '<?xml version="1.0" encoding="'.$to.'" ?>';
    my $text = &hash_to_xml( undef, $tree );
    if ( $from && $to ) {
        my $stat = &encode_from_to( \$text, $from, $to );
        croak "Unsupported encoding: $to\n" unless $stat;
    }
    join( "\n", $decl, $text );
}
# ----------------------------------------------------------------
sub parsehttp {
    ...
    croak "LWP::UserAgent is required: $@\n" if $@;
    eval { require HTTP::Request; } unless defined $HTTP::Request::VERSION;
    croak "HTTP::Request is required: $@\n" if $@;
...
sub parse {
    ...
    if ( $to ) {
        ...
        if ( $from ) {
            ...
            croak "Unsupported encoding: $from\n" unless $stat;
        }
    }
...
sub flat_to_tree {
    ...
    while( scalar @$source ) {
        ...
        if ( $node->{endTag} ) {
            ...
            croak "Invalid tag sequence: <$parent></$name>\n";
        }
...
sub hash_to_xml {
    ...
    my $out = [];
    my $attr = [];
    foreach my $key ( sort grep { /^-/} keys %$hash ) {
    ...
    foreach my $key ( sort grep {!/^-/} keys %$hash ) {
        my $val = $hash->{$key};
        if ( ref $val eq "ARRAY" ) {
            my $child = &array_to_xml( $key, $val );
            push( @$out, $child );
        } elsif ( ref $val ) {
            my $child = &hash_to_xml( $key, $val );
            push( @$out, $child );
        } elsif ( $val ne "" ) {
            my $child = &scalar_to_xml( $key, $val );
            push( @$out, $child );
        } else {
            push( @$out, "<$key />" );
        }
...
sub array_to_xml {
    ...
    foreach my $val ( @$array ) {
        if ( ref $val eq "ARRAY" ) {
            my $child = &array_to_xml( $name, $val );
            push( @$out, $child );
        } elsif ( ref $val ) {
            my $child = &hash_to_xml( $name, $val );
            push( @$out, $child );
        } elsif ( $val ne "" ) {
            my $child = &scalar_to_xml( $name, $val );
            push( @$out, $child );
        } else {
            push( @$out, "<$name />\n" );
        }
    ...
    if ( defined $Encode::VERSION ) {
        ...
        if ( $jfrom && $jto ) {
            ...
            if ( defined $Jcode::VERSION ) {
                ...
                croak "Encode.pm or Jcode.pm is required: $from to $to\n";
            }
        } else {
            croak "Encode.pm is required: $from to $to\n";
        }
    }
    $to;
}
